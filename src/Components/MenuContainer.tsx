  ...
  return (
    <div className="flex items-center justify-start mb-4">
      ...
      <div className="flex-none">
        <img className="w-32 h-32 object-cover rounded-l-xl" src={data?.image} alt="image" />
      </div>
      {/* Details */}
      <div className="flex-grow bg-white rounded-r-xl overflow-hidden shadow-lg">
        <div className="px-4 py-3">
          ...
          {data?.description && (
            <>
              <p className={`text-sm text-gray-700 mb-2 ${expanded || data?.description.length <= 100 ? 'block' : 'overflow-hidden h-12'}`}>
                {data?.description}
              </p>
              {data?.description.length > 100 && (
                <span className="text-blue-500 font-semibold cursor-pointer" onClick={toggleDescription}>
          ...
          <div className='flex justify-end items-center mt-2'>
            {count === 0 ? (
              <button className="bg-gray-800  text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out transform hover:scale-105 rounded-bl-xl rounded-tr-xl" onClick={handleAddToCart}>
                Add to Cart
              </button>
            ) : (
              <div className="flex items-center">
                <button className="bg-gray-800 text-white font-bold py-2 px-4 rounded-tl-xl rounded-bl-xl" onClick={handleDecrement}>-</button>
                <p className="mx-2">{count}</p>
                <button className="bg-gray-800 text-white font-bold py-2 px-4 rounded-tr-xl rounded-br-xl" onClick={handleIncrement}>+</button>
              </div>
            )}
          </div>
        </div>
